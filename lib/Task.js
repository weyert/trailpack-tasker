  ...
  requeueTaskForRetry(taskName) {
    return new Promise((resolve, reject) => {
      ...
      if (shouldRetryMessage) {
        ...
        this.app.log.silly(`Retrying the failed task, retry count: ${retryCount}/${maxRetries}, retrying? ${shouldRetryMessage}, waitingTimeBeforeQueueing: ${messageDelay} (${delayInMinutes} mins) previousMessageDelay: ${previousMessageDelay} originalExpiration: ${expiration}`);
        ...
        this.app.log.silly(`The total of retries (${retryCount}/${maxRetries}) for this task have been executed. Rejecting the message finally.`);
        this.reject();
        resolve()
      }
    })
