...
module.exports = class Task {
  ...
  ack() {
    if (!this.isAcknowledged) {
      ...
      this.app.log.debug('Attempting to ack a message that already responded')
    }
  }
  ...
  nack() {
    if (!this.isAcknowledged) {
      ...
      this.app.log.debug('Attempting to nack a message that already responded')
    }
  }
  ...
  reject() {
    if (!this.isAcknowledged) {
      ...
      cthis.app.log.debug('Attempting to reject a message that already responded')
    }
  }
  ...
  requeueTaskForRetry(taskName) {
    return new Promise((resolve, reject) => {
      ...
      const retryFactor = this.app.tasker.retryFactor;

      const shouldRetryMessage = retryCount <= maxRetries - 1;

      if (shouldRetryMessage) {
        console.log(`(${retryFactor}^${retryCount}) * ${retryDelay}`);
        const messageDelay = Math.pow(retryFactor, retryCount) * retryDelay;
        ...
        this.app.log.debug(`Retrying the failed task, retry count: ${retryCount}/${maxRetries}, shouldRetryMessage: ${shouldRetryMessage}  waitingTimeBeforeQueueing: ${messageDelay} (${delayInMinutes} mins) expectedRetryDelivery: ${expectedRetryDelivery} previousMessageDelay: ${previousMessageDelay} originalExpiration: ${expiration}`);
