...
exports.registerRun = function (taskName, app, rabbit) {
  ...
  rabbit.handle(taskName, message => {
    ...
    app.log.debug('Total worker count: %s Total concurrent tasks: %s', app.workerCount, app.config.tasker.concurrentTasks);
    if (app.workerCount >= app.config.tasker.concurrentTasks) {
      app.log.debug('Sending {nack}-message as we ran out of available workers to process the task');
      return message.nack()
    }
    ...
    Promise.resolve()
      ...
      .catch(err => {
        ...
        return task.finalize()
          .then(() => {
            app.log.info(`[path-a] Update the worker counter to ${app.workerCount}`);
            app.workerCount--
            exports.clearHandler(taskerClient.activeTasks.get(taskName), task)
          })
          .catch(() => {
            app.log.info(`[path-b] Update the worker counter to ${app.workerCount}`);
            app.workerCount--
            exports.clearHandler(taskerClient.activeTasks.get(taskName), task)
          })
      })
